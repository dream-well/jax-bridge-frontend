...
async function approve() {
    ...
    if((await check_allowance()) != true)
        approve_token("WJXN", contract, contract_addresses.jaxBridgeEVM, maxUint);
}

async function check_allowance() {
    let contract = new web3.eth.Contract(erc20ABI, contract_addresses.wjxn);
    let allowance = await callSmartContract(contract, "allowance", [accounts[0], contract_addresses.jaxBridgeEVM]);
...
async function update_status() {
    let contract = new web3.eth.Contract(jaxBridgeABI, contract_addresses.jaxBridgeEVM);
